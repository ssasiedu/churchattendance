...
function rememberKey(serviceId) {
  return `checkin:${serviceId}`
}
...
export default function CheckIn() {
  ...
  useEffect(() => {
    ...
    try {
      setMine(localStorage.getItem(rememberKey(data.service.id)))
    } catch {
      /* storage unavailable */
    }
  }, [data?.service])
  ...
  async function confirm() {
    ...
    if (result === 'closed') {
      setMessage({ tone: 'error', text: 'Check-in for this service has closed. Please see an usher.' })
      setSelected(null)
      load()
      return
    }
    if (result === 'invalid') {
      setMessage({ tone: 'error', text: 'That name is no longer on the list. Please see an usher.' })
      setSelected(null)
      load()
      return
    }
    ...
    setData((d) => ({
      ...d,
      members: d.members.map((m) => (m.id === selected.id ? { ...m, present: true } : m)),
    }))
    try {
      localStorage.setItem(rememberKey(data.service.id), selected.full_name)
    } catch {
      /* ignore */
    }
    ...
    setMessage({
      tone: 'ok',
      text: result === 'already' ? `${selected.full_name} was already marked present.` : `Welcome, ${selected.full_name.split(' ')[0]}! You’re marked present.`,
    })
    setSelected(null)
    setQuery('')
    setLetter(null)
    ...
    return (
      <CenteredCard>
        ...
        <p className="mt-2 text-slate-600">
          There’s no service accepting check-ins right now. If the service has started, please let an usher know.
        </p>
  ...
  return (
    <div className="mx-auto min-h-screen max-w-2xl bg-white sm:my-6 sm:min-h-0 sm:rounded-2xl sm:border sm:border-slate-200">
      ...
      <header className="bg-pew-900 px-5 pt-6 pb-5 text-white sm:rounded-t-2xl">
        <p className="text-sm text-pew-200">{CHURCH_NAME}</p>
        <h1 className="mt-1 font-display text-3xl leading-tight">{service.title}</h1>
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-sm">
          <span className="text-pew-100">{formatDate(service.service_date, { weekday: 'long', day: 'numeric', month: 'long' })}</span>
          <span className="rounded-full bg-pew-700 px-3 py-1 text-brass-300">
      ...
      {mine && (
        <div className="flex items-center gap-3 border-b border-pew-100 bg-pew-50 px-5 py-3 text-sm text-pew-700">
          ...
          <span>
            This phone checked in <strong>{mine}</strong>. Checking in someone else? Find their name below.
          </span>
        </div>
      )}
        ...
        <div className="relative">
          ...
          {query && (
            <button
              ...
              className="absolute top-1/2 right-3 -translate-y-1/2 rounded-full p-1.5 text-slate-500 hover:bg-slate-200"
          ...
          <li key={m.id}>
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                {m.department && <span className="block truncate text-sm text-slate-500">{m.department}</span>}
              </span>
              {m.present ? (
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setSelected(null)}>Not me</Button>
            <Button onClick={confirm} loading={busy}>Mark present</Button>
          </>
        }
      >
      ...
      {/* Result */}
      <Modal open={!!message} onClose={() => setMessage(null)} title={message?.tone === 'ok' ? 'Checked in' : 'Not checked in'} size="sm"
...
function LetterChip({ active, onClick, children }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'h-9 min-w-9 shrink-0 rounded-lg px-2.5 font-display text-base transition-colors',
